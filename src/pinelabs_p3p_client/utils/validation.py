...
def validate_create_token_options(
    ...
    customer_reference = _required_text(options.customerReference or options.customerId)
    ...
    if customer_auth_mode == P3PCustomerAuthMode.CustomerKey:
        ...
        if not customer_reference and not mobile_number:
            raise ValueError("CreateTokenOptions: customerReference or mobileNumber is required when customerAuthMode is CLIENT_CREDENTIALS")
    else:
